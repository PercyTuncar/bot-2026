  ...
  async execute({ sock, msg, args, groupId, groupJid, userPhone, replyJid, member }) {
    ...
    if (!rewardId) {
      await sock.sendMessage(replyJid, 
        formatError('Debes especificar el ID de la recompensa\n\nUso: .redeem {rewardId}\n\nUsa .rewards para ver las recompensas disponibles')
      );
      return;
    }
    
    // Capturar notas del usuario (resto de argumentos)
    const userNotes = args.slice(1).join(' ');
    
    try {
      const userName = msg.pushName || userPhone;
      
      // Usar teléfono canónico del objeto member si está disponible (evita duplicados LID)
      // Si no hay member, el handler intentará resolverlo internamente.
      const canonicalPhone = member ? (member.phone || member.id) : userPhone;

      const result = await RedemptionHandler.requestRedemption(
        groupId,
        canonicalPhone, // Usar ID canónico
        userName,
        rewardId,
        userNotes
      );
      ...
      try {
        ...
        if (chat && chat.isGroup) {
          ...
          // Enviar al grupo mencionando admins
