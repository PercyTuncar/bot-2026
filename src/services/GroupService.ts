...
import { batchCreateMembers } from '../utils/batch-operations.js';
import logger from '../lib/logger.js';

export class GroupService {
  /**
   * Crea o actualiza un grupo con TODOS los metadatos de WhatsApp
   * SPEC: Section 3.1 - Extracción COMPLETA de metadatos del grupo
   * @param {GroupChat} chat - Objeto GroupChat de whatsapp-web.js
   * @returns {Promise<Object>} - Datos del grupo creados/actualizados
   */
  static async createOrUpdateGroup(chat) {
    ...
    try {
      ...
      // Obtener metadatos completos del grupo
  ...
  /**
   * Extrae TODOS los metadatos disponibles de un GroupChat
   * SPEC: Section 3.1 - Estructura completa del documento groups
   */
  static async extractCompleteMetadata(chat) {
    const groupId = normalizeGroupId(chat.id._serialized);
    const now = getNow();
    
    // Obtener grupo existente para preservar datos históricos
    const existingGroup = await GroupRepository.getById(groupId);
    
    // ============ IDENTIFICACIÓN (de GroupChat) ============
    const metadata = {
      id: groupId,
      name: chat.name || 'Sin nombre',
      
      // ============ METADATOS DEL GRUPO (de GroupChat) ============
      description: chat.description || '',
      owner: chat.owner?._serialized || chat.owner || null,
      creationTimestamp: chat.createdAt || chat.timestamp || null,
      
      // ============ CONFIGURACIÓN DEL GRUPO (de GroupChat) ============
      isReadOnly: chat.isReadOnly || false,
      announce: chat.announce || false,
      restrict: chat.restrict || false,
      
      // ============ INVITACIÓN (de GroupChat) ============
      inviteCode: null, // Se obtiene por separado con getInviteCode()
  ...
  /**
   * Verifica si el bot es administrador del grupo
   */
  static isBotAdmin(chat) {
    // En whatsapp-web.js, necesitamos verificar si el bot está en la lista de participantes como admin
    // Esto requiere tener el número del bot
    return false; // Por ahora retorna false, se puede mejorar con el número del bot
  }
  ...
  static async activateGroup(groupId, groupMetadata, sock = null) {
    try {
      ...
      // CRITICAL: Obtener estado actual DIRECTO de BD (sin caché)
      ...
      const groupData: any = {
        ...
        // SPEC REQUIREMENT: Config must be inline (Section 3.1)
      ...
      // MODIFICACIÓN: Ya no registramos miembros masivamente.
      const membersRegistered = [];
      
      if (false && groupMetadata.participants && groupMetadata.participants.length > 0) { // DISABLED: Lazy registration
        ...
        for (const participant of groupMetadata.participants) {
          try {
            ...
            // Si es LID, intentar resolverlo via Contact
            ...
            // Crear estructura de miembro con datos iniciales
            const memberData = {
              phone: phoneNumber, // CLAVE PRIMARIA: número limpio (ej: "51959845515")
              displayName: participant.notify || phoneNumber, // Nombre público de WhatsApp
              ...
              // Sistema de puntos (inicializado en 0/1)
              points: 0,
              lifetimePoints: 0,
              messageCount: 0,
            ...
            // Guardar en Firestore usando phone como docId
  ...
  static async deactivateGroup(groupId) {
    groupId = normalizeGroupId(groupId); // NORMALIZE
