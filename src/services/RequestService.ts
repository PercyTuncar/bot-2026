...
export class RequestService {
   ...
   * @param {string} approvedBy - Teléfono del admin que aprueba
   * @param {object} sock - Cliente de WhatsApp para enviar notificaciones
   */
  static async approveRequest(requestId, approvedBy, sock = null) {
    ...
    // Notificar al usuario si hay cliente disponible
    if (sock && request.phone) {
      try {
        const userJid = phoneToJid(request.phone);
        const notificationMessage = 
          `✅ *Solicitud de Premio Aprobada*\n\n` +
   ...
   * @param {string} requestId - ID de la solicitud
   * @param {string} rejectedBy - Teléfono del admin que rechaza
   ...
   * Entrega un premio
   * @param {string} requestId - ID de la solicitud
   * @param {string} deliveredBy - Teléfono del admin que entrega
   * @param {object} sock - Cliente de WhatsApp para enviar notificaciones
   */
