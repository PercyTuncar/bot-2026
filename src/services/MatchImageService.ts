...
/**
 * MatchImageService - Servicio de generación de imágenes de match/pareja
 * 
 * Características:
 * - Dos fotos de perfil circulares con marco de corazones
 * - Título "¡MATCH DEL DÍA!"
 * - Corazón central entre las fotos
 * - Nombres debajo de cada foto
 * - Fecha y comando
 * - Diseño romántico con corazones decorativos
 */
class MatchImageService {
   ...
   * Usando formato PNG para compatibilidad directa con Sharp
   * API docs: https://api.multiavatar.com/{seed}.png
   */
   ...
   */
  async getProfilePicture(client: any, userId: string, userName: string): Promise<Buffer> {
    ...
    // Fallback a Multiavatar (ahora retorna PNG directamente)
    if (!avatarUrl) {
    ...
    try {
      ...
      // Fallback final - crear avatar simple con SVG
  ...
  /**
   * Crea un avatar fallback simple con las iniciales del usuario
   */
  private createFallbackAvatar(name: string): string {
  ...
  /**
   * Crea una imagen de match entre dos usuarios
   */
  async createMatchImage(
    user1: { id: string; name: string; jid: string },
    user2: { id: string; name: string; jid: string },
    client: any,
    ...
    try {
      ...
      // Descargar recursos en paralelo
      ...
      // Posiciones de los avatares (centrados horizontalmente con espacio entre ellos)
      const avatar1X = 170;  // Izquierda
      const avatar2X = 650;  // Derecha
      const avatarY = 340;   // Altura vertical

      // Procesar avatares a círculos
      const processAvatar = async (buf: Buffer): Promise<Buffer> => {
      ...
      // Crear el marco de corazones (anillo naranja/coral con corazones pequeños)
      const ringSize = avatarSize + 40;
      const createHeartRing = (): string => {
        const hearts: string[] = [];
      ...
      // Crear textos de nombres (limitados a 18 caracteres)
      ...
      // Si hay compatibilidad, agregar el porcentaje
