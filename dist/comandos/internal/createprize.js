...
export default {
    ...
    async execute({ sock, msg, args, groupId, isGroup, replyJid }) {
        ...
        if (!isGroup) {
            ...
            if (args.length < 4) {
                await sock.sendMessage(replyJid, formatError('Uso: .createprize [cÃ³digo] [nombre] [puntos] [descripciÃ³n]\n\n' +
        ...
        try {
            ...
            await sock.sendMessage(replyJid, formatSuccess(`Premio creado exitosamente\n\n` +
                `CÃ³digo: ${prize.code}\n` +
                `Nombre: ${prize.name}\n` +
