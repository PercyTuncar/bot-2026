        ...
        try {
            ...
            if (action === 'on') {
                ...
                try {
                    ...
                    await sock.sendMessage(replyJid, formatError('No se pudo acceder al grupo. Verifica que el bot esté en el grupo y que el ID sea correcto.'));
                    return;
                }
                if (!chat.isGroup) {
                ...
                try {
                    const groupMessage = `🤖 *BOT ACTIVADO*\n\n` +
                ...
                await sock.sendMessage(replyJid, response);
                try {
                    await sock.sendMessage(normalized, `⏸️ *Bot desactivado*\n\nEl bot ha sido desactivado en este grupo.`);
                }
                catch (error) {
                    logger.warn('No se pudo enviar mensaje al grupo:', error);
                }
