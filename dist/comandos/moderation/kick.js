    ...
    async execute({ sock, msg, args, groupId, groupJid, userPhone, replyJid }) {
        try {
            ...
            if (!target) {
                ...
                await reply(sock, msg, `${EMOJIS.ERROR} *Uso del comando:*\n\n` +
            ...
            if (!target.isLid && targetPhone === normalizedAdmin) {
                await reactError(sock, msg);
                await reply(sock, msg, `${EMOJIS.ERROR} No puedes expulsarte a ti mismo`);
                return;
            }
            if (chat && chat.isGroup) {
                try {
                    ...
                    if (participant?.isAdmin) {
                        await reactError(sock, msg);
                        await reply(sock, msg, `${EMOJIS.ERROR} 🛡️ No puedes expulsar a un administrador`);
                        return;
                    }
                }
                catch (e) {
            ...
            await sock.groupParticipantsUpdate(targetJid, [participantToKick], 'remove');
            ...
            await sock.sendMessage(targetJid, { text: confirmMessage, mentions: [participantToKick] });
            ...
            await reply(sock, msg, `${EMOJIS.ERROR} ${errorMessage}`);
